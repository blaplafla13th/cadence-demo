...
class GIP_Generator(nn.Module):
    ...
    def forward(self, x, m):
        ...
        z3 = self.fc3(h2)
        h3 = torch.sigmoid(z3)
        pdf_values = G(h2, h3, x.shape[0])

        # Compute the Negative Log-Likelihood (NLL)
        nll = -torch.log(pdf_values + 1e-10).mean()  # Adding a small value to prevent log(0)
        return h3, nll
...
class GIP:
    ...
    def impute(self):
        ...
        generator = GIP_Generator(num_features).to(device)
        ...
        for it in tqdm(range(iterations)):
            ...
            G_sample, _ = generator(X_mb, M_mb)  # Generate data using the generator
            ...
            G_sample, nll = generator(X_mb, M_mb)  # Generate data
            ...
            G_loss = G_loss_temp + alpha * MSE_loss  + nll # Combine losses
        ...
        with torch.no_grad():
            G_sample, _ = generator(X_mb, data_m)  # Generate final sample
